...
# We have b_r2 = b_s2 - mutant_selection[1]
...
mutant_time = [0.0]
mutant_selection = [0.0]

mutant_time2 = [Inf]
mutant_selection2 = [0.0]   # this will define fitness of resistant subclone during treatment

# other parameters
Nd = 10000
Nmut = 100
Nmax_vector = Nd*[1.5, 2.0, 2.5, 3.0]
#time_vector = [1.0,2.0,3.0,4.0]
...
function plot_SFS_linear(SFS_xaxis, SFS_simu, SFS_simu_single, SFS_pred)
    ...
    ax = Axis(f[1,1], #xscale=log10, yscale=log10,
        xlabel = "# of cells, 𝑘", xlabelsize = 25, xticklabelsize = 25,
        ylabel = "# of mutations, 𝑆ₖ", ylabelsize = 25, yticklabelsize = 25#,
        #xticks = ([1e0,1e1,1e2,1e3,1e4],[L"10^0",L"10^1",L"10^2",L"10^3",L"10^4"]),
        #yticks = ([1e0,1e1,1e2,1e3],[L"10^0",L"10^1",L"10^2",L"10^3"])
    )
...
for i in 1:realizations
    ...
    local input = BranchingInput(Nmax=Nd, birthrate=b_s1, deathrate=d_s1, μ=mu, ploidy=1)
    mutant_time[1] = age(simulation.output)
    selection = SelectionPredefined(mutant_selection, mutant_time)
    #simulation = runsimulation(SimpleTreeCell, input, selection)
    global simulation = runsimulation(simulation.output, SimpleTreeCell, WellMixed, input, selection)
    ...
    for (j, Nmax) in enumerate(Nmax_vector)
        ...
        local input = BranchingInput(Nmax=Nmax, tmax=t_detec+j*timestep, birthrate=b_s2, deathrate=d_s2, μ=mu, ploidy=1)
        local selection = SelectionPredefined(mutant_selection2, mutant_time2)
        global simulation = runsimulation(simulation.output, SimpleTreeCell, WellMixed, input, selection)
